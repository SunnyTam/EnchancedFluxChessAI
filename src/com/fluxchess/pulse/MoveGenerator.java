...
import static com.fluxchess.pulse.MoveType.NULLMOVE;
...
final class MoveGenerator {
  ...
  MoveList<MoveEntry> getNullMoves(@NotNull Position position) {
	  moves.size = 0;
	  int activeColor = position.activeColor;
	  int kingSquare = next(position.pieces[activeColor][KING].squares);
	  int kingPiece = position.board[kingSquare];
	  
	  moves.entries[moves.size++].move = Move.valueOf(NULLMOVE, kingSquare, kingSquare, kingPiece, NOPIECE, NOPIECETYPE);
	  return moves;
  }
