...
import com.fluxchess.pulse.TranspostionTable.TranspostionTableEntity;
import com.fluxchess.pulse.TranspostionTable.Bound;
...
final class Search implements Runnable {
  ...
  private final int nullMoveSearchReduce = 2;
  
  private TranspostionTable transpostionTable;
  ...
  public void run() {
    while (true) {
      ...
      transpostionTable = new TranspostionTable();
      ...
      for (int depth = initialDepth; depth <= searchDepth; ++depth) {
        ...
        searchRoot(currentDepth, -INFINITE, INFINITE,transpostionTable);
  ...
  private void searchRoot(int depth, int alpha, int beta, TranspostionTable transpostionTable) {
	int iidDepth = depth;
	transpostionTable.setAge(iidDepth);
    int ply = 0;
    ...
    for (int i = 0; i < rootMoves.size; ++i) {
      ...
      //create Transpostion Table


      position.makeMove(move);
      int value = -search(depth - 1, -beta, -alpha, ply + 1,false,transpostionTable,iidDepth);
  ...
  private int search(int depth, int alpha, int beta, int ply,boolean checkedNullMove,TranspostionTable currentTranspostionTable,int iidDepth) {
    // We are at a leaf/horizon. So calculate that value.
    if (depth <= 0) {
      // Descend into quiescent
      return quiescent(1, alpha, beta, ply, checkedNullMove);
    }
    ...
    //Check Transpostion Table
    boolean useTranspostionTable = true;
    if(useTranspostionTable){
    	TranspostionTableEntity currentEntity = currentTranspostionTable.getEntity(position,ply,iidDepth);
    	if (currentEntity != null){
    		int bounded = currentEntity.getBounded();
    		int value = currentEntity.getValue();
    		switch (bounded) {
            case Bound.LOWER:
              if (value >= beta) {
                return value;
  ...
  //null move
    if(!checkedNullMove ){
    	...
    	if (!position.isCheck(position.activeColor)){
    		position.makeMoveNull();
	    	int nullMoveValue = -search(depth - 1 - nullMoveSearchReduce, -beta, -beta + 1, ply + 1,true,currentTranspostionTable,iidDepth);
	    	position.undoMoveNull();
	    	
	    	if(nullMoveValue >= beta) {
	    		   if(useTranspostionTable){
    			      currentTranspostionTable.putEntity(position.zobristKey,nullMoveValue,depth,Bound.LOWER,iidDepth);
    			    }
	    		
	    		return nullMoveValue; // Cutoff
	    	}
	    	
    	}
    }
    
    int bounded = Bound.UPPER;
    ...
    for (int i = 0; i < moves.size; ++i) {
      ...
      if (!position.isCheck(opposite(position.activeColor))) {
        ++searchedMoves;
        value = -search(depth - 1, -beta, -alpha, ply + 1,checkedNullMove,currentTranspostionTable,iidDepth);
      }
      position.undoMove(move);
    ...
    /*if(transpostionValue != 0 && transpostionValue != bestValue){
      	System.out.println("transpostionValue = " + transpostionValue + "Value = " + bestValue);
      	System.out.println("Current Position Key" + position.zobristKey + "Transpotaion Key = " + currentEntity.getZobristkey());
      	System.out.println("Current Depth = " + ply + "Transpotaion Depth = " + currentEntity.getDepth());
      	}
      */
    
    //put into TranspostionTable
    if(useTranspostionTable){
      currentTranspostionTable.putEntity(position.zobristKey,bestValue,depth,bounded,iidDepth);
    
    currentTranspostionTable = null;
    }
  ...
  private int quiescent(int depth, int alpha, int beta, int ply) {
	  return quiescent(depth,alpha,beta,ply,true);
  }

  private int quiescent(int depth, int alpha, int beta, int ply,boolean checkedNullMove) {
    updateSearch(ply);
