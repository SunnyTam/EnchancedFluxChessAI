...
final class Evaluation {
  ...
  static int positioningWeight = 80;
  private static final int MAX_WEIGHT = 100;
  
  static int castlingScore=50;
  static int noneCastlingScore=-30;
  ...
  int evaluate(@NotNull Position position) {
    ...
    //int positioningScore = (PiecePostionEvaluator.evaluatePosition(myColor, position) - PiecePostionEvaluator.evaluatePosition(oppositeColor, position))
    //		* positioningWeight / MAX_WEIGHT;
  ...
  private int evaluateMaterial(int color, @NotNull Position position) {
    ...
    if(position.castlingRights == Castling.NOCASTLING){
    	material += noneCastlingScore;
    }else{
    	material += castlingScore;
    }
