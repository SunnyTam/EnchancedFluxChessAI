...
final class Position {
	...
	private static final class Zobrist {
		...
		static {
			...
			castlingRights[WHITE_KINGSIDE | WHITE_QUEENSIDE] = castlingRights[WHITE_KINGSIDE] ^ castlingRights[WHITE_QUEENSIDE];
			castlingRights[BLACK_KINGSIDE | BLACK_QUEENSIDE] = castlingRights[BLACK_KINGSIDE] ^ castlingRights[BLACK_QUEENSIDE];
	...
	boolean hasInsufficientMaterial() {
		...
		return pieces[WHITE][PAWN].size() == 0 && pieces[BLACK][PAWN].size() == 0 && pieces[WHITE][ROOK].size() == 0 && pieces[BLACK][ROOK].size() == 0 && pieces[WHITE][QUEEN].size() == 0 && pieces[BLACK][QUEEN].size() == 0 && (pieces[WHITE][KNIGHT].size() + pieces[WHITE][BISHOP].size() <= 1) && (pieces[BLACK][KNIGHT].size() + pieces[BLACK][BISHOP].size() <= 1);
	}
	...
	boolean isAttacked(int targetSquare, int attackerColor) {
		...
		return isAttacked(targetSquare, Piece.valueOf(attackerColor, KNIGHT), knightDirections)

		// The queen moves like a bishop, so check both piece types
				|| isAttacked(targetSquare, Piece.valueOf(attackerColor, BISHOP), Piece.valueOf(attackerColor, QUEEN), bishopDirections)

				// The queen moves like a rook, so check both piece types
				|| isAttacked(targetSquare, Piece.valueOf(attackerColor, ROOK), Piece.valueOf(attackerColor, QUEEN), rookDirections)

				|| isAttacked(targetSquare, Piece.valueOf(attackerColor, KING), kingDirections);
	}
	...
	int taperedEval() {
		int phase = taperedEvalPhrase(pawnCount,rookCount,bishopCount,knightCount,queenCount);
		return ((OPENING * (256 - phase)) + (ENDGAME * phase)) / 256;
	}

	int taperedEvalPhrase(int pawnCount, int rookCount, int bishopCount, int knightCount, int queenCount) {
		int pawnPhase = 0;
		int knightPhase = 1;
		int bishopPhase = 1;
		int rookPhase = 2;
		...
		phase -= pawnCount * pawnPhase; // the number of pawns currently on the board
